...
import cn.tf.project.service.CensorService;
...
//    }
     ...
     * @param param       参数列表（JSON格式字符串）
     * @return
     */
    @ApiOperation(value = "电子审图ASMX调用", notes = "电子审图ASMX调用接口", httpMethod = "POST")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "serviceName", value = "服务方法名称", required = true, dataType = "string"),
            @ApiImplicitParam(name = "param", value = "参数列表", required = true, dataType = "string"),
            @ApiImplicitParam(name = "serviceType", value = "接口类型（Interface,CheckDraw,Material）", dataType = "string")
    })
    @RequestMapping(value = "callCensorService")
    @ResponseBody
    public String callCensorService(@RequestParam(value = "serviceName") String serviceName,
                                  @RequestParam(value = "param") String param,
                                  @RequestParam(value = "serviceType", required = false) String serviceType) throws JSONException, AxisFault {
        JSONObject jsonObject = new JSONObject(param);
        JSONObject result = censorService.callCensorService(serviceName, jsonObject,serviceType);
        return result.toString();
    }
